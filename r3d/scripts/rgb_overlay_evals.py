...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Cap frames sent per question; default uses all frames in the window.",
    )
...
def _resolve_max_images(
    ...
    """Largest number of frames any question window will send to the model.

    With ``--max-frames`` set this is exactly that value; otherwise it is the
    biggest window across the evaluated annotations, so the vLLM per-prompt
    image limit always matches what we actually send (never disagrees).
    """
    if max_frames is not None:
        return max_frames
    ts_by_seq: dict[str, list[int]] = {}
    largest = 1
    for ann in annotations:
        seq = ann.identity_layer.sequence_id
        ql = ann.query_layer
        if seq not in ts_by_seq:
            ts_by_seq[seq] = frame_store.get_all_timestamps(seq)
        start = ql.query_timestamp_ns_start or 0
        end = ql.query_timestamp_ns_end or 0
        count = sum(1 for ts in ts_by_seq[seq] if start <= ts <= end)
        largest = max(largest, count)
    return largest


def _make_vlm(args: argparse.Namespace, max_images_per_prompt: int):
...
def main() -> None:
    ...
    max_images = _resolve_max_images(annotations, frame_store, args.max_frames)
    logger.info(f"vLLM per-prompt image limit set to {max_images}")
    vlm = _make_vlm(args, max_images)
