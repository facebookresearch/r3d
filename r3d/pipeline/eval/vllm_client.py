...
from r3d.types import ChatRole, Message
...
def _worker_loop(
    ...
    while True:
        ...
        if item is None:
            break
        caller_id, chat_messages, max_tokens = item
...
class VLLMClient(VLMClient):
    ...
    def query(
        ...
        return self._generate(chat_messages, max_tokens)
    ...
    def _generate(
        ...
        if self._workers is None:
            return self._generate_single(chat_messages, max_tokens)
        return self._generate_multi(chat_messages, max_tokens)
    ...
    def _generate_multi(
        ...
        self._request_queues[worker_rank].put((caller_id, chat_messages, max_tokens))
        event.wait()
        with self._lock:
