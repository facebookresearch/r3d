...
def _apply_object_filter(
    ...
    """Apply the static/has-dynamic/all object filter."""
    if object_filter == "all":
        return filtered_objects
    if object_filter == "static":
        keep = [obj for obj in filtered_objects if obj.is_static]
    elif object_filter == "has-dynamic":
        keep = [obj for obj in filtered_objects if not obj.is_static]
    else:
        raise ValueError(f"Unknown object_filter: {object_filter}")
    if verbose:
        logger.info(
            f"  Object filter '{object_filter}': "
            f"{len(filtered_objects)} -> {len(keep)} objects"
        )
    return keep


def _filter_objects_for_generation(
    all_video_objects: list,
    gt_provider: object,
    ...
    """Filter objects by visibility/distance/FOV, then names and object filter."""
    if verbose:
    ...
    return _apply_object_filter(filtered_objects, object_filter, verbose)


def _run_question_generators(
    ...
    """Run each question type's registered generator and collect annotations."""
...
def generate_v2_annotations(
    ...
    # 2. Filter and refine objects
    filtered_objects = _filter_objects_for_generation(
        all_video_objects=all_video_objects,
        gt_provider=gt_provider,
        timestamps_ns=timestamps_ns,
        stream_id=stream_id,
    ...
    all_annotations = _run_question_generators(
...
def _resolve_frames_db(frames_db_arg: str | None) -> Path | None:
    """Resolve the optional --frames-db argument to a validated frames.db path."""
    if not frames_db_arg:
        return None
    p = Path(frames_db_arg)
    if p.is_dir():
        p = p / "frames.db"
    if not p.exists():
        raise RuntimeError(f"frames.db not found at {p}")
    return p


def _validate_object_library(
    args: argparse.Namespace,
    question_types: list[QuestionType] | None,
) -> None:
    """Fail loudly if volume question types are requested without --object-library."""
...
def _process_sequence(
    ...
    args: argparse.Namespace,
    question_types: list[QuestionType] | None,
    disambiguation_method: DisambiguationMethod | None,
    frames_db_local: Path | None,
    store: SQLiteAnnotationStore,
    output_dir: Path,
    video_paths: dict[str, str],
) -> tuple[str, list[Annotation]]:
    """Generate, persist, and (optionally) visualize annotations for one sequence."""
    sequence_name = Path(sequence_path).name
    logger.info(f"=== Processing sequence: {sequence_name} ===")

    gt_provider, _data_paths = load_sequence_data(sequence_path, verbose=not args.quiet)

    if frames_db_local is not None:
        timestamps_ns = _load_timestamps_from_frames_db(frames_db_local, sequence_name)
    else:
    ...
    return sequence_name, annotations
...
def main(argv: list[str] | None = None) -> None:
    ...
    _validate_object_library(args, question_types)
    ...
    frames_db_local = _resolve_frames_db(args.frames_db)
    ...
    for sequence_path in args.sequences:
        sequence_name, annotations = _process_sequence(
            sequence_path=sequence_path,
            args=args,
            question_types=question_types,
            disambiguation_method=disambiguation_method,
            frames_db_local=frames_db_local,
            store=store,
