...
- [Quick Start - Running Your Model On R3D-Bench](#quick-start---running-your-model-on-r3d-bench)
  - [Standard VLM (RGB baseline)](#standard-vlm-rgb-baseline)
...
### Standard VLM (RGB baseline)

To run a standard VLM (RGB-only, no depth inputs) on R3D-Bench, use `r3d.scripts.rgb_overlay_evals`. Add `--overlay` to additionally paint the referenced objects' SAM3 masks onto the frames, and `--score` to score immediately after generation:
...
You can modify this evaluation command to run your own VLM that accepts depth and pose as input. The example above with visual overlays demonstrates how to use SAM3 masks in evaluation.
...
Run a VLM on R3D-Bench. Two backends are supported: vLLM (recommended) and HuggingFace.

#### vLLM backend (recommended)

vLLM provides faster inference with automatic KV cache reuse across tool-use turns.
...
For single-GPU inference or models not supported by vLLM:
...
python -m r3d.pipeline.scripts.generate_responses \
    ...
    --hf-model Qwen/Qwen3-VL-8B-Instruct \
    --backend hf \
    --output-dir /tmp/r3d/responses_hf
```
